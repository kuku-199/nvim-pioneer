...
> **Nvim Pioneer** 快捷键快速参考指南  
> 💡 **提示：** 在 Neovim 中按 `<Space>` 可查看交互式快捷键列表
...
1. [通用与窗口管理](#-1-通用与窗口管理)
2. [文件与终端](#-2-文件与终端)
3. [代码开发 (LSP)](#-3-代码开发-lsp)
4. [调试 (DAP)](#-4-调试-dap)
5. [通用快捷键模式](#️-5-通用快捷键模式)
...
| :--- | :--- | :--- |
| `<Space>` | Leader | **主功能键** (先按这个) |
| `<Space>sv` | `:vsplit` | 垂直分屏 (左右分割) |
| `<Space>sh` | `:split` | 水平分屏 (上下分割) |
| `<Space>h` | `<C-w>h` | 光标移向左窗口 |
| `<Space>j` | `<C-w>j` | 光标移向下窗口 |
| `<Space>k` | `<C-w>k` | 光标移向上窗口 |
| `<Space>l` | `<C-w>l` | 光标移向右窗口 |
| `<Space>nh` | `:nohl` | 清除搜索高亮 |
| `<Space>th` | `:SwitchTheme` | 切换并保存主题 |
| `<Space>tm` | `:ToggleMonitor` | 开关底部状态监控 (CPU/RAM) |
| `jj` | `<Esc>` | 快速退出插入模式 |
...
> ⚡ **注意：** 这些快捷键仅在语言服务器激活时有效（例如打开 `.py`、`.lua`、`.rs` 文件后）。
...
> 🔧 **注意：** 调试功能需要为你的语言配置 DAP。

| 按键 (Key) | 动作 (Action) | 说明 (Description) |
| :--- | :--- | :--- |
| `<F5>` | Continue | 开始/继续调试 |
| `<F10>` | Step Over | 单步跳过 (不进入函数) |
| `<F11>` | Step Into | 单步进入 (进入函数) |
| `<F12>` | Step Out | 单步跳出 (退出函数) |
...
| 按键 (Key) | 命令 (Command) | 说明 (Description) |
| :--- | :--- | :--- |
| `<Space>mp` | `MarkdownPreviewToggle` | 开关浏览器预览 |
| `<Space>mr` | `RenderMarkdown toggle` | 开关编辑器内渲染 |

---

<div align="center">

**💡 专业提示：** 使用 `:WhichKey` 或按 `<Space>` 后稍等，可查看交互式快捷键弹出窗口！
